...
[English](https://github.com/ExcuseLme/JHenTai/blob/master/README.md) | 简体中文
| [한국어](https://github.com/ExcuseLme/JHenTai/blob/master/README_kr.md)

[常见问题，提问前必看](https://github.com/jiangtian616/JHenTai/wiki/%E5%B8%B8%E8%A7%81%E9%97%AE%E9%A2%98)

## 本fork改动

本fork基于原版进行了个人操作偏好定制改动, 仅提供android和windows版.

### 新增功能

- 网络连接超时后自动重试：在网络较差的环境下，连接超时或数据读取失败后会自动重试。配置项：'设置-网络'下的'网络连接失败自动重试次数'和'重试间隔'。
- 收藏按钮长按取消收藏：在画廊详情页中，已收藏的画廊可以长按收藏按钮直接取消收藏。
- 长按触发时长可配置：各种长按操作的触发时间现在可以自定义。配置项：'设置-高级'下的'长按触发时长'。
- 动图加载性能提升：GIF/APNG/WebP 动态图的加载和播放更加流畅。
- 精简右键菜单：画廊详情页中标题、上传者的右键菜单现在只显示应用自身的功能，不再显示系统或其他应用的菜单项。
- 自动语言过滤扩展：在画廊详情页中选中文字搜索时，也会自动应用语言过滤设置。
- 回顶按钮半透明：快速回顶按钮现在显示为半透明，减少对内容的遮挡。

### 优化

- 搜索响应更快：在搜索页等待结果时修改条件重新搜索，会立即打断旧搜索执行新搜索。
- 本地画廊封面修复：本地画廊列表的封面图现在正确显示为按名称排序的第一张图片。
...
[<img src="https://raw.githubusercontent.com/ExcuseLme/JHenTai/master/badges/get_it_on_obtainium.png" 
