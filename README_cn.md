...
新增功能
- 网络连接超时后自动重试功能: 在质量较差的网络中经常出现网络连接超时或数据读取超时, 该功能可在此类失败后自动重试. 对应配置项: '设置页-网络'下的'网络连接失败自动重试次数'和'重试间隔'.
- 收藏按钮长按触发取消收藏: 在画廊页中, 如果已收藏该画廊, 长按收藏按钮触发取消收藏.
- 长按触发时长: 各种长按触发的事件所需的长按时间现在可配置. 对应配置项: '设置页-高级'下的'长按触发时长'.
优化
- 执行新搜索时打断未完成的旧搜索: 在搜索页中等待搜索结果时, 修改搜索条件重新触发搜索后会打断未完成的旧搜索, 执行新搜索
